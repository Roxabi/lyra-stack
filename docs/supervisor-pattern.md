...
### Systemd system services (infrastructure, not user-managed)

Some infrastructure components run as **system** services (not user units) and are managed independently of supervisord.

| Service | Unit file | Purpose |
|---------|-----------|---------|
| `nats` | `/etc/systemd/system/nats.service` | NATS message broker — pub/sub hub for Lyra workers |

NATS is **opt-in** — installed by `make nats-install` (not `provision.sh`). Single-machine setups skip it.
`make nats-install` installs the binary, system user, and config at `/etc/nats/nats.conf`.
Auth: nkey public-key auth. Transport: TLS 1.3. Binds to LAN interface only (port 4222).
Certs and nkeys are generated by `scripts/gen-nats-certs.sh` and `scripts/gen-nats-nkeys.sh`.

The `lyra-stack.service` user unit starts after `nats.service` via a drop-in at
`~/.config/systemd/user/lyra-stack.service.d/after-nats.conf` (installed by `make nats-install`).
