...
NATS is installed by `scripts/provision.sh` (binary + system user + config at `/etc/nats/nats.conf`).
...
`~/.config/systemd/user/lyra-stack.service.d/after-nats.conf` (installed by `provision.sh`).
